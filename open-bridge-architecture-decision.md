...
- CLI 通过 `chrome.runtime.connectNative()` 与扩展建立双向通信
- 使用 JSON-RPC 2.0 协议，通过 stdin/stdout 传输
- 扩展是"胖客户端"，包含完整的会话管理、CDP 封装、光标覆盖层、Favicon 状态指示
- 三层重连保障：setTimeout + chrome.alarms + storage.session 持久化
...
| 会话管理 | 通过标签页租约或会话边界控制不同任务拥有的标签页 |

**劣势**：

| 方面 | 说明 |
|------|------|
| 封闭性 | Native Host ID 硬编码，只有 CodeX CLI 能连接 |
| 部署复杂 | 需要在系统中注册 Native Messaging Host 配置文件 |
| 不可扩展 | 工具定义在 Rust CLI 端，其他 AI 工具无法接入 |
| 平台限制 | 需要为每个操作系统处理本地程序分发 |
...
```
AI 工具 (Cursor/Claude) ──本地接口/MCP── 本地 daemon ──WebSocket── Chrome Extension
```

**核心机制**：

- **守护进程**：WebSocket Server + 会话管理 + 工具调度
- **扩展**：WebSocket Client + 工具注册表 + CDP 执行器
...
- 提供较完整的 MCP 工具集合，覆盖输入自动化、导航、性能分析、网络、调试、内存等场景
- 配置极简：一行 JSON 即可使用
...
OpenBridge 当前支持的工具列表，涵盖常见的浏览器控制能力：
...
守护进程默认监听本地回环地址，并通过 runtime 状态记录实际端口，不暴露到局域网或公网。
...
| **扩展** | TypeScript + WXT 框架 | WXT 是现代浏览器扩展开发框架，支持 HMR、TypeScript、多浏览器 |
...
| 部署方式 | ✅ npm / install.sh 一键启动 | 需要平台相关构建或分发 |
| 包体积 | 依赖随 npm 安装解析 | 需要单独考虑平台产物分发 |
